...
export class AuthService {
  ...
  public async authenticate(auth: LoginPayload, user: UserProxy): Promise<Partial<UserEntity>> {
  ...
  public async validateUserByPayload(jwtPayload: IJwtPayload): Promise<UserEntity> {
    ...
    const user = await this.usersService.findOne(jwtPayload.id.toString()).catch(() => null);
