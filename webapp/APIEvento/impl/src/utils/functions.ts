...
export function ensureJoin(crudRequest: CrudRequest, join: string): CrudRequest {
  crudRequest.parsed.join = [...crudRequest.parsed.join, {
    field: join,
  }];
  return crudRequest;
}
 ...
 * Exemplo:
 * ```TypeScript
 * const userPayload = await validatePayload(CreateUserLikePayload, { name: 'Joga10' });
